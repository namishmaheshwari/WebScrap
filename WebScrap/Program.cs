    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            using (var driver = new ChromeDriver(@"C:\Users\chill\Downloads\WebScrap"))
            {
                ...
                foreach (IWebElement element in elements)
                {
                    ...
                    for (int i = 1; i <= elementsStream.Count;i++){
                        ...
                        for (int majorCount = 1; majorCount <= elementsMajor.Count;majorCount++){
                            ...
                            var elem = driver.FindElementByXPath(selectorMajor + "[" + majorCount + "]");

                            var js1 = (IJavaScriptExecutor)driver;
                            js1.ExecuteScript("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'})", elem);

                            //this one will scroll the element into view for interactions
                            IJavaScriptExecutor je1 = (IJavaScriptExecutor)driver;
                            je1.ExecuteScript("arguments[0].scrollIntoView(false);", elem);
                            ...
                            for (int studentCount = 1; studentCount <= elementsStudent.Count; studentCount++)
                            {
                                ...
                                var eles = driver.FindElementByXPath(selectorStudent + "[" + studentCount + "]");

                                var js = (IJavaScriptExecutor)driver;
                                js.ExecuteScript("arguments[0].scrollIntoView({behavior: 'smooth', block: 'center'})", eles);

                                //this one will scroll the element into view for interactions
                                IJavaScriptExecutor je = (IJavaScriptExecutor)driver;
                                je.ExecuteScript("arguments[0].scrollIntoView(false);", eles);
                                webDriver.ClickElement(driver, By.XPath(selectorStudent + "[" + studentCount + "]"), 1000);
                                driver.FindElementByXPath(selectorStudent + "[" + studentCount + "]/div").Click();
                ...
                System.IO.File.WriteAllText(@"D:/student-details-new.txt", string.Join("\n", schools));
